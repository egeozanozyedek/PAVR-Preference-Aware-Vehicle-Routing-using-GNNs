...
#     M.addConstr(sum(x[:,0]) <= n_vehicles )
    ...
    M.setObjective( sum(x[:,i] @ cst_matrix[:,i] for i in range(n)), GRB.MAXIMIZE)
    M.optimize()
    if M.status==2:
        cmnt = "optimal"
...
if __name__ == "__main__":
    a = eval_sd([[0,4,3,2,0], [0,1,0]], [[0,3,1,2,0], [0,4,0]])
    print(a)